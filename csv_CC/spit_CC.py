...
save_path = "./"
csv_file = "/data/users/lhx/csv/CC_6/camera_catalogue.csv"
...
info_common_file = os.path.join(save_path, "CC_common_info.csv")
info_all_file = os.path.join(save_path, "CC_all_info.csv")
        ...
        with open(csv_file_path, "r", newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                if species:
                    ...
                    # 处理没有标签的行
                    print(f"Missing label in row: {row}")
    except Exception as e:
...
def write_csv(file_path, rows, header):
    """train和val中只保留前9个常见物种"""
    with open(file_path, "w", newline='') as file:
        writer = csv.writer(file)
        writer.writerow(header)
        for row in rows:
            if int(row[1]) > 8:  # 确保只写入前9个物种
                continue
            writer.writerow(row)
...
def process_data(rows, species_counts, species_rank):
    ...
    for r in rows:
        ...
        if rank is not None:
            species_data[label].append([r[0], rank])
